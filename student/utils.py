...
def triple_sigmoid_E(s, tx, ty, alfa_x, alfa_y, bx, by, be, ex, ey, angle_e):
    ...
    y = 1.0 / np.hstack([1.0 + np.exp(-sR * np.hstack([by, bx])), 1.0 + np.exp(
...
def get_color_prob(ima, positions=None, patchSize=1):
    ...
    for k in range(numChromatics):
        ...
        CD[:, k] = (np.double(beta_e != 0.0) * triple_sigmoid_E(np.vstack((a, b)), tx, ty, alfa_x, alfa_y, beta_x,
